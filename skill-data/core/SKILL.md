...
`snapshot -i` only returns interactive elements (buttons, textboxes, tabs, etc.). To read **display-only text** (labels, status text, percentages, hints), use `snapshot` without `-i`:

```bash
seeless-uia snapshot --json
# Look for "text" role entries in the output:
#   - text "剩余用量 45%" [ref=e114]
# Then read its content:
seeless-uia get text e114
# "1 周45%6月4日"
```

If you don't know the exact ref, use `get text` with a selector that targets the general area:
```bash
seeless-uia get text status:StatusBar   # Read status bar text
seeless-uia get text control:Text        # First Text element (non-interactive labels)
```

**Note:** `get text` reads the element's TextPattern content (visible text + formatting). For simple text elements it returns the Name string. The reading is recursive — adjacent text children may be merged.
...
```bash
seeless-uia click e5
seeless-uia snapshot -i --json
# Compare with previous snapshot — look for:
#   - New elements appeared (dialog, popup, menu)
#   - Element state changed (checked/unchecked, expanded/collapsed)
#   - Value changes (textbox content)
```

**When snapshot shows no change:**
- The element may not support InvokePattern (UIA click). Try `hover e5` + `click e5` sequence.
- The interaction may open a new window. Run `seeless-uia windows` to check.
- Visual-only changes (colors, animations) are not reflected in UIA tree.
- If the element is in a different window, use `seeless-uia click w3 e5` to target it explicitly.

For text input verification, `get value` is more reliable than snapshot:
```bash
